...
%time_lima=datenum(1985,1,1,0,0,0):1:datenum(2022,12,31,0,0,0); % GLOBALWAVE-ERA5
...
correct_model=-1; % -1=scatter_kde, 0=binscatter, 1=correct it
...
for fobs=stations
  ...
  if plot_rose
    ...
    for expt=expts
      ...
      for i=1:length(dcol)
        ...
        Options = [Option, {'axes', ax, 'cmap', 'invbone','TitleString', {[replace(file,'_',' '), ' obs. ',tnames{dcol(i)}];''},...
        ...
        Options = [Option, {'axes', ax, 'cmap', 'invbone','TitleString', {expt;''},...
  ...
  if plot_disp
    ...
    for expt=expts
      ...
      tnames={'Peak Period (s)',    'Peak Direction (^o)',    'Directional spread (^o)',   'Tm01 (s)','Tm02 (s)','Hs (m)','Qp (^o)','Tm (s)','Water Level (m)'};
      for i=1:length(dcol)
        ...
        % printing minimum and maximum xplot and yplot values
				display(['Min and max xplot: ',num2str([min(xplot) max(xplot)])])
				display(['Min and max yplot: ',num2str([min(yplot) max(yplot)])])
        ...
        if correct_model==-1;
          ...
          hold on
          plot([0 20],[0 20],'--','color',[.7 .7 .7],'linewidth',2)
          ...
          if ke==length(expts)
          	...
          	%set(h,'position',[0.63 0.11 0.09 0.215735294117647]) % [left bottom width height]
          	set(h,'position',[0.585 0.22 0.3 0.015]) % [left bottom width height]
            set(get(h,'ylabel'),'string','Probability density estimate','fontsize',12,'fontweight','bold');
          else
            colorbar('off')
          end
          %colorbar
        ...
        title([lett{ke},' ',expt])% ,': ',replace(file,'_',' '),' ',tnames{dcol(i)}])

        if fobs==1
          xlim([3.5 14.5]); ylim([3.5 14.5])
          %xlim([0 16]); ylim([0 16])
          xd=5; yd=14; dsz=1;
          ctext='b';
        elseif fobs==2
        ...
        if correct_model<=0;
          %text(1,yd,['y=',num2str(p(1),'%.2f'),'*x',num2str(p(2),'%+.2f')],'fontsize',10,'fontweight','bold');
          text(xd,yd-dsz,['RMSE=',num2str(mod_rmse,'%.2f'),' s']                ,'fontsize',10,'fontweight','bold','color',ctext);
          text(xd,yd-dsz*2,['bias=',num2str(mod_bias,'%.2f'),' s']              ,'fontsize',10,'fontweight','bold','color',ctext);
          text(xd,yd-dsz*3,['r=',num2str(mod_corr,'%.2f')]                 ,'fontsize',10,'fontweight','bold'     ,'color',ctext);
        elseif correct_model==1;
          %  legh=[legh,{[expt,' rmse=',num2str(mod_rmse,'%.2f'),' corr=',num2str(mod_corr,'%.2f'),', bias=',num2str(mod_bias,'%.2f')]}];
