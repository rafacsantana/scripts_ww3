...
time_lima=datenum(2021,1,10,0,0,0):1:datenum(2021,1,14,23,0,0);    % Baring head wave direction, time series
...
for fobs=stations
  ...
  if plot_series
        ...
        if plot_obs==1 
          ...
          if (dcol(i)~=2 || dcol(i)~=1) & i==1 || i==2
            ...
            if add_stats
              ...
              elseif dcol(i)==4
                legt=[legt,{[expt,' rmse=',num2str(mod_rmse,'%.2f'),' corr=',num2str(mod_corr,'%.2f'),', bias=',num2str(mod_bias,'%.2f')]}];
              else
                legt=[];%[legt,{[expt]}];%,' rmse=',num2str(mod_rmse,'%.2f'),' corr=',num2str(mod_corr,'%.2f'),', bias=',num2str(mod_bias,'%.2f')]}];
              end
            else
    ...
    %  subplot(length(dcol),1,find(dcol==6));
      ...
      %if i==1; legfinal=legh; else; legfinal=legt; end
      subplot(length(dcol),1,1);
      legloc={'NE','SW','SW'};
      legend([legh],'location',legloc{stations},'fontsize',8)

      if stations==2
        subplot(length(dcol),1,3);
        legend('RiCOM','TELEMAC','location','NW','fontsize',8)
      else
        subplot(length(dcol),1,3);
        legend('RiCOM','location','NW','fontsize',8)
        subplot(length(dcol),1,2);
        legend([legt],'location','SW','fontsize',8)
      end
  ...
  system(['mkdir -p ',path_dm]);
  figname=[path_dm,file,'_tides_',replace(tnames{dcol(i)},' ','_'),'_',datestr(time_lima(1),'YYYY_mm_DD_HH_MM'),'_',datestr(time_lima(end),'YYYY_mm_DD_HH_MM'),'.png'];
