...
% Daily time
time_lima=datenum(1986,1,1,0,0,0):1:datenum(1995,12,31,0,0,0);    % hindcast
%time_lima=datenum(1996,1,1,0,0,0):1:datenum(2002,12,31,0,0,0);    % hindcast
%time_lima=datenum(2008,1,1,0,0,0):1:datenum(2013,12,31,0,0,0); %
%time_lima=datenum(2015,1,1,0,0,0):1:datenum(2022,12,31,0,0,0);    % hindcast

time_lima=datenum(2007,1,1,0,0,0):1:datenum(2007,12,31,0,0,0);    % hindcast

%time_lima=datenum(2020,12,1,0,0,0):1:datenum(2020,12,15,0,0,0);  % 1-year experiments
%time_lima=datenum(2021,6,1,0,0,0):1:datenum(2021,12,31,0,0,0);  % 1-year experiments
...
elseif stations==1
  time_lima=datenum(2021,10,21,0,0,0):1:datenum(2021,10,21,0,0,0); % Banks Peninsula head tide and direction
  %time_lima=datenum(2021,10,22,0,0,0):1:datenum(2021,10,22,0,0,0); % Banks Peninsula head tide and direction
end

%time_lima=datenum(2029,12,1,0,0,0):1:datenum(2030,1,1,0,0,0); % climate projections

files={'Banks_Peninsula','Baring_Head','wairewa_lake_forsyth','Pelorus_Sound','Taumutu','Mangawhai'}; 
%stations=length(files);
...
plot_series=0; % stations=0, plot_series=1, plot_coastm=1, plot_obs==0 to save the coastal points
plot_rose  =0;
plot_disp  =0;
plot_eva   =0;

plot_coastm=0; % statistical maps for coastal points
plot_map   =1; % daily station maps
...
dcol=[6,1,2];%,4,11]; % Hs, wlv

% maps
pcname ='vort'; % m_pcolor: wlv, hs, curr, vort
  ...
  if plot_series
    ...
    for expt=expts
      ...
      if save_mat==1
        ...
        display(['Saving: ',fda]);
        save(fda,'time_mod','tp_mod','pd_mod','ds_mod','tm01_mod','tm02_mod','hs_mod','tm_mod','wlv_mod') % ,'ucur_mod','vcur_mod','we')
      end
...
return
  ...
  if plot_map
    ...
    for t=time_lima
      ...
      if fobs==1
	      hourly=1; % 1 = hourly, 2 = 2-hourly
      else
	      hourly=1; % 1 = hourly, 2 = 2-hourly
      end
			if fobs==1
        ih=length(time)/2 + 1; hr=2; fh=length(time); % it goes in the interval between 1 and length(time)
	      %subl=8; subc=3;        
        [subp,subs]=create_expanded_subp(4,3,4); % (squared matrix, ncols, nlines);
      elseif fobs==2 % baring head
        %ih=2; hr=4; fh=length(time);
        ih=3; hr=2; fh=length(time)/2+2; % it goes in the interval between 1 and length(time)
	      %subl=4; subc=3;        
        %[subp]=create_expanded_subp(3,4,3);
        [subp,subs]=create_expanded_subp(4,3,4); % (squared matrix, ncols, nlines);
      end
      ...
      for i=ih:hr:fh 
        ...
        for expt=expts
          ...
          ax(ke)=subplot(size(subs,1),size(subs,2),subp(kt,:));
          %ax(ke)=subplot(1,length(expts),ke);
          hold on
          if fobs==1
            m_proj('mercator','long', [lon_obs-.5 lon_obs+.25],'lat',[lat_obs-.25 lat_obs+.25]);
          else
            m_proj('mercator','long', [lon_obs-.36 lon_obs+.1],'lat',[lat_obs-.1 lat_obs+.2]);
          end
          ...
          elseif strcmp(pcname,'vort')
            ...
            % relative vorticity
            dubar_mod=u_mod(:,2:end)-u_mod(:,1:end-1); % in x direction
            dvbar_mod=v_mod(2:end,:)-v_mod(1:end-1,:); % in y direction
            dubar_mod=u2rho_2d(dubar_mod);
            dvbar_mod=v2rho_2d(dvbar_mod);

            dubardy=dubar_mod./2000;
            dvbardx=dvbar_mod./2000;
            omega=7.292E-5;
            vor_mod=(dvbardx-dubardy); % ./abs(grd.f);
            %vor_mod=(dvbardx-dubardy)./(2.*omega.*sind(lat_modm'); % ./abs(grd.f);
          ...
          %caxis([0 .13])
          if kt==1 % 3
            cb=colorbar; set(get(cb,'ylabel'),'string','','fontsize',12,'fontweight','bold'); set(cb,'fontsize',12,'fontweight','bold');
            set(cb,'position',[.92 .11 .02 .814]); % [left bottom width height]
          end
          ...
          if strcmp(expt,'NZWAVE-HR')
            [cs,h]=m_contour(lon_mod,lat_mod,-depth_mod',[-30 -80],'color',[.7 .7 .7],'linewidth',2);
          ...
          if kt==10          
	          m_grid('fontname','helvetica','fontsize',8,'fontweight','bold');
					elseif kt==1 || kt==7 || kt==4
	          m_grid('xticklabels',[],'fontname','helvetica','fontsize',8,'fontweight','bold');
					elseif kt==11 || kt==12
	          m_grid('yticklabels',[],'fontname','helvetica','fontsize',8,'fontweight','bold');
          else
          	%%m_grid('xtick',[round(loni):2:round(lonf)],'xticklabels',[round(loni):2:round(lonf)],'fontname','helvetica','fontsize',20,'fontweight','bold');
	          m_grid('xticklabels',[],'yticklabels',[],'fontname','helvetica','fontsize',8,'fontweight','bold');
          end
          ...
          if fobs==1
            title([lett{kt},' ',datestr(time(i),'HH:MM dd/mmm/yyyy')],'fontsize',8,'fontweight','bold','color','b')
          else
            title([lett{kt},' ',datestr(time(i),'HH:MM dd/mmm/yyyy')],'fontsize',8,'fontweight','bold')
          end
          ...
          if strcmp(vecname,'hs')
            m_vec(vsize,lon_modm(1:v_spa:end,1:v_spa:end),lat_modm(1:v_spa:end,1:v_spa:end),u_mod(1:v_spa:end,1:v_spa:end)',v_mod(1:v_spa:end,1:v_spa:end)','k','shaftwidth',.75,'headwidth',3,'headangle',60,'headlength',3.5)
          ...
          elseif strcmp(vecname,'curr')
            m_vec(vsize,lon_modm(1:v_spa:end,1:v_spa:end),lat_modm(1:v_spa:end,1:v_spa:end),u_mod(1:v_spa:end,1:v_spa:end)',v_mod(1:v_spa:end,1:v_spa:end)','k','shaftwidth',.75,'headwidth',3,'headangle',60,'headlength',3.5)
            if fobs==1
              [hp ht]=m_vec(vsize,172+(58/60),-43-(47/60),1,0,'k','shaftwidth',.75,'headwidth',3,'headangle',60,'headlength',3.5,'key',[num2str(1.0),' m/s']);
              set(ht,'fontsize',8,'fontweight','bold')
            else
              [hp ht]=m_vec(vsize,174+(42/60),-41-(19/60),1,0,'k','shaftwidth',.75,'headwidth',3,'headangle',60,'headlength',3.5,'key',[num2str(1.0),' m/s']);
              set(ht,'fontsize',8,'fontweight','bold')
            end
          end
          ...
          if fobs==1
            ...
            m_vec(vsize,lon_mod(ilon),lat_mod(ilat),u_mod(ilon,ilat),v_mod(ilon,ilat),'b','shaftwidth',2,'headwidth',6,'headangle',60,'headlength',8)
	          m_vec(vsize,lon_obs,lat_obs,u_obsc(i),v_obsc(i),obs_vcol             ,'shaftwidth',2,'headwidth',6,'headangle',60,'headlength',8)
            m_text(172+(49.9/60),-43-(43/60),['Mod=',num2str(hs_mod(ilon,ilat),'%.2f'),'m'],'colo','b','fontsize',8,'fontweight','bold') 
            m_text(172+(50/60),-43-(45/60),['Obs=',num2str(hs_obs(i),'%.2f'),'m'],'colo',[1 0 1],'fontsize',8,'fontweight','bold') 
					else
      	    vsize=3;
            m_vec(vsize,lon_mod(ilon),lat_mod(ilat),u_mod(ilon,ilat),v_mod(ilon,ilat),'b','shaftwidth',1.5,'headwidth',6,'headangle',60,'headlength',8)
	          m_vec(vsize,lon_obs,lat_obs,u_obsc(i),v_obsc(i),obs_vcol                     ,'shaftwidth',1.5,'headwidth',6,'headangle',60,'headlength',8)
            m_text(174+(38/60),-41-(16/60),['Mod=',num2str(pd_mod(ilon,ilat),'%.1f'),'^o'],'colo','b','fontsize',8,'fontweight','bold') 
            m_text(174+(38/60),-41-(18/60),['Obs=',num2str(pd_obs(i),'%.1f'),'^o'],'colo',[1 0 1],'fontsize',8,'fontweight','bold') 
          end
