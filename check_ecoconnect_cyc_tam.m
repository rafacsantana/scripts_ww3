...
time_lima=datenum(2019,1,1,0,0,0):1:datenum(2019,1,31,0,0,0); % Graham Harrington ECAN request on 18/09/2023
%time_lima=datenum(2020,1,6,0,0,0):1:datenum(2020,12,31,0,0,0); % Graham Harrington ECAN request on 18/09/2023
%time_lima=datenum(2021,1,6,0,0,0):1:datenum(2021,12,31,0,0,0); % Graham Harrington ECAN request on 18/09/2023
%time_lima=datenum(2022,1,6,0,0,0):1:datenum(2022,12,31,0,0,0); % Graham Harrington ECAN request on 18/09/2023
%time_lima=datenum(2023,1,1,0,0,0):1:datenum(2023,12,31,0,0,0); % Graham Harrington ECAN request on 18/09/2023

%time_lima=datenum(2018,1,6,0,0,0):1:datenum(2020,12,31,0,0,0); % Graham Harrington ECAN request on 18/09/2023

%time_lima=datenum(2021,1,1,0,0,0):1:datenum(2021,1,4,0,0,0); % Graham Harrington ECAN request on 18/09/2023
%time_lima=datenum(2021,1,1,0,0,0):1:datenum(2024,4,30,0,0,0); % Graham Harrington ECAN request on 18/09/2023
...
path_fig=['/scale_wlg_persistent/filesets/project/niwa03150/reevegm/rafa_ww3_dataread/figures/']; % GLOBALWAVE/'];%2018/01/05/00'];
...
for s=1:length(stations)
  ...
  %filename='bla';
  display(['Looking for: ',filename])%,'time_mod','model')
  ...
  if check_file==1 && exist([filename])==2
    ...
    for t=time_lima
      ...
      display(['Looking for: ',fileday])%,'time_mod','model')
...
%   csvhead={'PeakPeiod'       ,'PeakDirection'      ,'Uwind(m/s)'                  ,'Vwind(m/s)'    ,'None1','Hs'                       ,'MeanDirection'};
    ...
    for i=1:length(dcol)
      ...
      elseif dcol(i)==6
        ...
        yyaxis right
        plot(time,model.data(:,8)','.','color',[0 .5 0],'linewidth',2)
        ylim([0 360])
        set(gca,'YColor',[0 .5 0]);
