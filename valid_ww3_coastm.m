...
%time_lima=datenum(1999,1,1,0,0,0):1:datenum(1999,12,31,0,0,0);    % 1-year 
time_lima=datenum(2003,1,1,0,0,0):1:datenum(2003,12,31,0,0,0);    % 1-year 
time_lima=datenum(2013,1,1,0,0,0):1:datenum(2013,12,31,0,0,0);    % 1-year 
%time_lima=datenum(2023,1,1,0,0,0):1:datenum(2023,12,31,0,0,0);    % 1-year 
%time_lima=datenum(2020,1,1,0,0,0):1:datenum(2020,12,31,0,0,0);    % 1-year 
...
files={'Banks_Peninsula','Baring_Head'            ,'wairewa_lake_forsyth','Pelorus_Sound'  ,'Taumutu',... 5 
       ...
       'Blenheim'       ,'Mokohinau'              ,'Pukehina'            ,'Taharoa'}; ... 14
...
plot_cycs  =2; % 1 all cyclones 2 = strong cyclones. plot cyclone initial and final dates onn the time series plot (plot_series)
save_csv   =0;
save_mat   =1;
save_fig   =0;
save_video =0;
...
if plot_aserie || plot_amap || correct_model==2
  ...
  atype='cmems_l4_05deg'; % for wave height correction before EVA
  % cmems_l4  
...
if plot_coastm || stations(1)==0
  ...
  %figure; hold on; imagesc(depth_mod'); colorbar; caxis([0 50])
  ...
  %hold on; 
  %%imagesc(lon_mod,lat_mod,depth_mod'); shading flat; colorbar; caxis([0 400])
  %%contour(lon_mod,lat_mod,depth_mod',[50 50],'r');
  %%plot(lon_mod(points(:,1)),lat_mod(points(:,2)),'.k'); xlim([165 180]); ylim([-50 -30])
  %%plot(lon_modm(i50),lat_modm(i50),'.r'); 
...
%return
          ...
          % uncomment all at once to real useful stuff
    ...
    for expt=expts
      ...
      for i=ccol
        ...
        if i==1 % max wave height
          ...
          return_p=1;
          [data,eva]=GPDfitwaves_funcion(hs_mod,time_mod,1,return_p); % =GPDfitwaves_funcion(hs_obs,time_obs,kind,rp)
