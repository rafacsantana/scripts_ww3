...
time_lima=datenum(2009,1,1,0,0,0):1:datenum(2013,12,31,0,0,0); %
time_lima=datenum(2016,1,1,0,0,0):1:datenum(2022,12,31,0,0,0);    % hindcast

%time_lima=datenum(1983,1,1,0,0,0):1:datenum(1983,12,31,0,0,0);    % hindcast
...
% altimeter stats
acol=[1]; % 1 = sdt and rmse, 3 = mean and bias, %%%% selected variables from avalm
%acol=[7:12];
%series
crop_l4=0;
      ...
      if plot_amap
        ...
        for i=1:length(acol)
          ...
          if plot_obs==1 & ka==1
            ...
            h=colorbar;
            set(h,'position',[0.49 0.709338925400994 0.0222686151704941 0.216159504916794]) % [left bottom width height]
            if acol(i)==3
              set(get(h,'ylabel'),'string','Mean satellite Hs (m)','fontsize',10,'fontweight','bold','color',[1 1 1]);
              set(get(h,'ylabel'),'position',[.2 2.75000166893005 0]);
            elseif acol(i)==1
              set(get(h,'ylabel'),'string','Satellite Hs SD (m)','fontsize',10,'fontweight','bold','color',[1 1 1]);
              set(get(h,'ylabel'),'position',[.2 (2+.5)/2 0]);
            end
            ...
            title(['Satellite Hs (mean=',num2str(nanmean(obsdata(:)),'%.2f'),'m)']);
...
%zzz
          ...
          if ka==5
            h=colorbar;
            set(h,'position',[0.49 0.1 0.0222686151704941 0.525]) % [left bottom width height]
            if acol(i)==3
              set(get(h,'ylabel'),'string','Sig. Wave Height forecast bias (m)','fontsize',10,'fontweight','bold','color',[0 0 0]);
              set(get(h,'ylabel'),'position',[.2 7.15255737304688e-07 0]);
            elseif acol(i)==1
				      set(get(h,'ylabel'),'string','Sig. Wave Height forecast RMSE (m)','fontsize',10,'fontweight','bold','color',[0 0 0]);
	    			  set(get(h,'ylabel'),'position',[.2 (1-0)/2 0]);
				    end
          end
          %[cs,h]=m_contour(lon_mod,lat_mod,-depth_mod',[-1000 -1000],'color',[1 1 1],'linewidth',2);
          %clabel(cs,h,'color',[0 0 0],'fontsize',10,'fontweight','bold','LabelSpacing',2000)
          ...
          title([expt,' (mean=',num2str(nanmean(data(:)),'%.2f'),'m)']);
