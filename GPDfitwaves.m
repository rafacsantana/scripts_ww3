...
cdfplot(hs_obs) % find suitable thresholds to constrain GPthres.m. This is just a rough guide. looks like something between 2 and 6 metres would work
...
[GPD,gpp,gpci,Sy,ARI_POT,POT_sorted] = GPDfit(hs_obs+tolerance,time_obs,tthr,ythr,ARI);

figure(2), clf, semilogx(ARI_POT,POT_sorted,'ok'), hold on
...
semilogx(ARI,GEV(1,:),'k','LineWidth',2), semilogx(ARI,GEV(2,:),'--k',ARI,GEV(3,:),'--k'), hold off
title('GEV fitted to annual maxima'), xlabel('Average recurrence interval  (years)'), ylabel('H_s')

figure(2), hold on, semilogx(ARI,GEV(1,:),'b'), semilogx(ARI,GEV(2,:),'--b',ARI,GEV(3,:),'--b'), 
