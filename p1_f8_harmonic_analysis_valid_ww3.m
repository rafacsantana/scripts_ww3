...
% Daily time
time_lima=datenum(1988,1,1,0,0,0):1:datenum(1995,12,31,0,0,0);    % hindcast
time_lima=datenum(1998,1,1,0,0,0):1:datenum(2002,12,31,0,0,0);    % hindcast
%time_lima=datenum(2016,1,1,0,0,0):1:datenum(2022,12,31,0,0,0);    % hindcast
%time_lima=datenum(2014,1,1,0,0,0):1:datenum(2015,12,31,0,0,0);    % hindcast

time_lima=datenum(1991,1,1,0,0,0):1:datenum(1991,12,31,0,0,0);    % hindcast
time_lima=datenum(1998,1,1,0,0,0):1:datenum(1998,12,31,0,0,0);    % hindcast
%time_lima=datenum(2018,1,1,0,0,0):1:datenum(2018,12,31,0,0,0);    % hindcast

%time_lima=datenum(2020,12,1,0,0,0):1:datenum(2020,12,15,0,0,0);  % 1-year experiments
time_lima=datenum(2021,1,1,0,0,0):1:datenum(2021,2,28,0,0,0);    % 1-year 
...
files={'Banks_Peninsula','Baring_Head','wairewa_lake_forsyth','Pelorus_Sound','Taumutu','Mangawhai','BoP_leatherback_turtles'}; 
stations=length(files);
stations=[2]; % 0 = coastal stations (points)
...
% altimeter stats
acol=[1]; % selected variables from avalm
%acol=[7:12];
%series
crop_l4=0;
